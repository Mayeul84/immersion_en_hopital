...
myeloid_cells = neyman_subsample(data=myeloid_cells, target_labels=["Myeloid cells"], label_col="celltype_label", stratify_by=["cellstates_tme"], n_target=int(n_cells))
myeloid_cells = update_data(data=myeloid_cells)

# UMAP 
print("[3/3] Computing UMAP.")
plot_UMAP(myeloid_cells)
plt.savefig(os.path.join(SAVE_DIR, "UMAP_myeloid.png"), dpi=200, bbox_inches='tight')
plt.close()
...
study_complete_sparsity(
    myeloid_cells, labels=labels, ratio_candidates=ratio_candidates_full, 
...
study_complete_sparsity(
    myeloid_cells, labels=labels, ratio_candidates=ratio_candidates_low, 
    n_runs=n_runs, n_neighbors_candidates=n_neighbors_candidates, 
...
# --- EXPE 7 ---
print("\n" + "="*50)
print("EXPERIENCE 6/8: Group Sparsity OneVsAll (Ratio 0.01 -> 0.25, k=20)")
print("="*50)
t0_exp7 = time.time()
study_group_sparsity_onevsall(
    myeloid_cells, labels=labels, ratio_candidates=ratio_candidates_low, 
